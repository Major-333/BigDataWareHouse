...
BEGIN
    IF larger = 'greater' THEN
        select product_id, title, emotion_score_emotion_score, score_score, time_time_id
        from movie
...
BEGIN
    select D2.director_name, D1.product_id
    from director D1
...
BEGIN
    select director_name, product_id
    from actor_movie
             natural join movie
             natural join director
    where actor_name = act;
...
BEGIN
    select A2.actor_name, A1.product_id
    from actor_movie A1
             join actor_movie A2
